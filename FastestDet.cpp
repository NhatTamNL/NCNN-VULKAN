...
int fasetest_detect(cv::Mat srcImg, const char* model_bin, const char* model_param, std::vector<TargetBox> &dst_boxes)
{
...
int main(int argc, char **argv) {
    ...
    cv::Mat srcImg = cv::imread("3.jpg");

    std::vector<TargetBox> target_boxes;

    // testCam(model_bin, model_param, video_path);
    double start = ncnn::get_current_time();
    // fasetest_detect(model_param, model_bin, target_boxes);
    
    std::vector<TargetBox> nms_boxes;
    fasetest_detect(srcImg, model_bin, model_param, nms_boxes);
